...
def entrada():
    usuario = str(entrada_usuario.get()).strip()
...
def entrar_tela_administrador():
    ...
    call(['python', 'telas\\tela_administrativa.py'])
...
if not verificação(): # type: ignore
    janela_login.destroy()
    caixa_de_mensagem = messagebox.showerror(title='ERRO', message='Não encontrado o banco de dados')
