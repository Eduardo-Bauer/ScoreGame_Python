...
def nova_senha():
    nome = str(entrada_usuario.get()).strip()
    senha = str(entrada_senha.get()).strip()
    if nome == '' or senha == '':
...
texto_usuario = ttk.Label(janela_nova_senha, text='Digite o Usuário', style='TLabel')
...
#botoes#
sensura = tk.IntVar()
botao_sensura = ttk.Checkbutton(janela_nova_senha, text = 'Mostrar senha', style = "TCheckbutton", variable = sensura, onvalue = 1, offvalue = 0, command = mostrar_senha)
...
#botao de entrada#
botao_entrar.grid(row=6, column=0, padx=(50, 0), pady=(25, 0), ipadx= 70)
